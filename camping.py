...
class Reservation(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        info_button = ttk.Button(self, text="𝓲＊",
                            command=self.info,width=3).grid(row=13, column=6)
    ...
    def info(self):
        self.info_label = tk.Label(self, text="1) each room can accommodate up to 4 people\n2) children underage deserve a discount\t", font=("Times New Roman", 10),
                                      background='#D8C4B6').grid(row=13, column=6)
    ...
    def enter_data(self):
        ...
        if Q3 is None:
            ...
            Q3 = Q3[0]
        Accomodations= (children1*2+adults1*4)+Q1+(children2*2+adults2*4)+Q2+(children3*2+adults3*4)+Q3

        self.total_cost = (Y001_kostos+Y002_kostos+Y003_kostos+Accomodations)*tDays.days
        ...
        date_insert_tuple_plhrwmh = (f"P00{count_plhrwmh + 1}",(count+1),f"KR0{count_kratisi + 1}",self.total_cost )
        cursor.execute(data_insert_query_plhrwmh, date_insert_tuple_plhrwmh)

        conn.commit()
        conn.close()
        self.successReserv()
    def successReserv(self):
        success_label = tk.Label(self, text="Successful Reservation",
                                 background='#997C70', foreground="black",
                                 font=("Times New Roman", 15))
        success_label.grid(row=1,column=6,pady=15)
        self.image = PhotoImage(file="success.png")
        self.image = self.image.subsample(10,10)
        labelImage = tk.Label(self, image=self.image, bg='#D8C4B6')
        labelImage.grid(row=2,column=6)
        cost_label = tk.Label(self, text="Total Cost from Reservation",
                              background='#D8C4B6', foreground="black",
                              font=("Times New Roman", 15))
        cost_label.grid(row=3,column=6)
        money_label = tk.Label(self, text=f":{self.total_cost}€",
                              background='#D8C4B6', foreground="black",
                              font=("Times New Roman", 15, "bold"))
        money_label.grid(row=3, column=7)
...
class DatabaseRecords(tk.Frame):
    def __init__(self, parent, controller):
        ...
        back_button = ttk.Button(self, text="Receptionist", command=lambda: controller.show_frame("Receptionist"))
        back_button.pack()

        back_button = ttk.Button(self, text="Welcome", command=lambda: controller.show_frame("Visitor"))
        back_button.pack()

        for table in tables:
            # Create a frame for each table
            frame = tk.Frame(notebook,bg='#D8C4B6')
            ...
            delete_button = ttk.Button(frame, text="Delete Selected Record", command=lambda tree=tree, table=table: self.delete_selected_record(tree,table))
            delete_button.pack()

        conn.commit()
        conn.close()

    def delete_selected_record(self, tree,table):
        ...
        pk_col = cursor.description[0][0]  # First column name
        # Delete the record from the database
        cursor.execute(f"DELETE FROM {table} WHERE {pk_col} = ?", (pk,))
        conn.commit()
        conn.close()
